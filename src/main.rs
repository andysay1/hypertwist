...
fn simulate_gravitational_lensing() -> std::io::Result<()> {
    ...
    // Нормировка: g_{μν} dx^μ dx^ν = 0 для светоподобных траекторий (фотоны)
    {
        ...
        let inner_product = gdot[(0, 0)];

        // Проверяем, что траектория почти светоподобная
        if inner_product.abs() < 1e-10 {
            // Уже светоподобная, нормировку не меняем
            println!("Траектория уже светоподобная: gμνdxμdxν = {}", inner_product);
        } else if inner_product > 0.0 {
            // Времениподобная - нужно увеличить пространственные компоненты
            println!("Нормируем времениподобную траекторию: gμνdxμdxν = {}", inner_product);
            // Сохраняем временную компоненту
            let dt = dx[0];
            // Масштабируем пространственные компоненты
            let scale = ((inner_product.abs()) / (-g[(0, 0)] * dt * dt)).sqrt();
            for i in 1..4 {
                dx[i] *= scale;
            }
        } else {
            // Пространственноподобная - нужно увеличить временную компоненту
            println!("Нормируем пространственноподобную траекторию: gμνdxμdxν = {}", inner_product);
            // Масштабируем временную компоненту
            let space_part = inner_product - g[(0, 0)] * dx[0] * dx[0];
            dx[0] = (-space_part / g[(0, 0)]).sqrt();
        }
